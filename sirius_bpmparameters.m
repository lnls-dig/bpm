...
cable = struct(...
    ...
    'fe', 500e6, ...                   % Characteristic frequency of the cable [Hz]
...
chamber = struct(...
    'type', 'circular', ...
    'radius', 18.1e-3 ...                 % Vacuum chamber radius [m]
);

button = struct(...
    'type', 'round', ...
    'diameter', 12e-3, ...               % Button diameter [m]
    'gap', 0.5e-3, ...                  % Button gap [m]
    'thickness', 2.4e-3, ...              % Button thickness [m]
    'R0', 50, ...                       % Load impedance [ohm]
    'Cb_meas', 2.6e-12 ...             % Measured capacitance [F]
);
